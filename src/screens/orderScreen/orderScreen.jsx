...
const OrderScreen = () => {
  ...
  const manualPayPalHandler = () => {
    if (!paypal?.clientId) {
      toast.error("Configura tu PAYPAL_CLIENT_ID para habilitar el pago");
      return;
    }
    setShowPayPalButton(true);
    paypalDispatch({ type: "setLoadingStatus", value: "pending" });
              ...
              {order.orderItems.map((item, index) => (
                <ListGroup.Item key={index} className="list-item">
                  <Row style={{ alignItems: "center" }}>
                    ...
                    <Col md={4}>
                      {item.qty} x Bs {Number(item.price).toFixed(2)} = Bs {" "}
